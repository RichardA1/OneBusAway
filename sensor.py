...
class OneBusAway(Entity):
    ...
    def async_update(self):
        ...
        for arrival in response_object_data['entry']['arrivalsAndDepartures']:
            arrivalObject = OneBusAwayBusArrival(arrival)
            if arrivalObject.route_id == self._route_id and arrivalObject.number_of_stops_away > 0:
                self._state = arrivalObject.get_next_bus_time()
                self._state_attributes = arrivalObject.get_state_attributes()
                break
...
class OneBusAwayBusArrival(object):
    ...
    def get_state_attributes(self):
        ...
        state_attributes = {
            ...
            ATTR_SCHEDULED_ARRIVAL_TIME: OneBusAway.get_time_string(
